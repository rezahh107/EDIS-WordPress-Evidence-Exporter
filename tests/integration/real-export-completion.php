...
$postId = wp_insert_post([
    'post_type' => 'page',
    'post_status' => 'publish',
    'post_title' => 'EDIS 3.7.16 Real Export Gate',
    'post_content' => '',
], true);
...
try {
    ...
    if (($stored['implementation_version'] ?? null) !== '3.7.15') {
        throw new RuntimeException('Fresh job implementation_version is not the preserved 3.7.15 compatibility value.');
    }
    ...
    if (!is_array($stored) || ($stored['implementation_version'] ?? null) !== '3.7.15') {
        throw new RuntimeException('Completed job implementation_version is not the preserved 3.7.15 compatibility value.');
    }
    ...
    if (($manifest['producer']['version'] ?? null) !== '3.7.16'
        || ($manifest['data']['bundle_schema_version'] ?? null) !== '3.3.0'
        || ($validation['data']['evidence']['state'] ?? null) !== 'PASS') {
        throw new RuntimeException('Completed ZIP metadata does not prove the 3.7.16/Schema 3.3.0 PASS contract with preserved 3.7.15 worker compatibility.');
    }
