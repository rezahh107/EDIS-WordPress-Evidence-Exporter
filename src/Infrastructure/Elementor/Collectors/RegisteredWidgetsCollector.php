...
final class RegisteredWidgetsCollector implements EvidenceCollector
{
    ...
    public function collect(CollectionContext $context, array $artifacts = []): CollectionResult
    {
        ...
        if (!is_array($types)) {
            throw new \RuntimeException('Elementor widget registry API returned an unusable value.');
        }
        $rows = [];
        foreach ($types as $key => $widget) {
            ...
            $rows[] = [
                'name' => $name,
                'title' => is_object($widget) && method_exists($widget, 'get_title') ? (string) $widget->get_title() : null,
                'categories' => is_object($widget) && method_exists($widget, 'get_categories') ? array_values(array_map('strval', (array) $widget->get_categories())) : [],
        ...
        return new CollectionResult(
            ...
            ['widgets' => $rows, 'count' => count($rows)],
            [],
            [],
            ['collector_id' => $this->id(), 'adapter_id' => 'elementor.widgets-manager', 'adapter_version' => '1.1.0', 'source_kind' => 'ELEMENTOR_REGISTRY', 'retrieval_strategy' => 'get_widget_types'],
        );
    }
}
