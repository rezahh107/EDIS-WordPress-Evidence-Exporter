...
Version `3.7.13`, generated by EDIS Enterprise Deterministic Build Platform `3.7.13`.

Version 3.7.13 is a bounded degraded-admin recovery patch. It restores a minimal `EDIS Evidence` Diagnostics / Recovery surface when the existing startup gates enter fail-closed degraded mode, without initializing the operational export graph. Healthy `AdminModule` behavior is unchanged, frozen evidence contracts remain unchanged, and worker implementation compatibility deliberately remains `3.7.12`.
...
`Bootstrap` remains the sole authority that decides healthy versus degraded execution. Every current degraded branch routes through the shared `DegradedModeIntegration`, which now registers a minimal recovery-only wp-admin surface under `manage_options` using the existing `EDIS Evidence` menu identity and Diagnostics slug.
...
Degraded mode does **not** initialize `AdminModule`, `DiagnosticsService`, export services, operational stores, worker/recovery execution services, or operational REST controllers. Create Export, job operations, downloads, worker tests, Inspector operational APIs, settings requiring operational storage, and scheduled export execution remain unavailable until the existing runtime, integrity, configuration and private-storage gates pass. On a later healthy request, the normal `AdminModule` returns and the degraded shell is not registered.
...
Version 3.7.0 introduced the frozen EDIS-CJ-2 profile; version 3.7.13 preserves it unchanged:
